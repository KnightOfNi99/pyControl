...
class G(object):
    ...
    '''
    def meander_arc(self, x, y, spacing, start='LL', orientation='x', tail=False,
                minor_feed=None):
        """ Infill a rectangle with an arc wave meandering pattern. If the
        relevant dimension is not a multiple of the spacing, the spacing will
        be tweaked to ensure the dimensions work out.
        ...
        for _ in range(int(self._meander_passes(minor, spacing))):
            ...
            if orientation == 'x':
                ...
                if dirCW:
                    ...
                    self.arc(x=0, y=(spacing/2), direction='CW')
                    dirCW = False
                
                else:
                    self.arc(x=0, y=(spacing/2), direction='CCW')
                    dirCW = True
            else:
                
                if dirCW:
                
                    self.arc(x=(spacing/2), y=0, direction='CW')
                    dirCW = False
                
                else:
                    self.arc(x=(spacing/2), y=0, direction='CCW')
                    dirCW = True
